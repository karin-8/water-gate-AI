...
if "gates" not in st.session_state:
    st.session_state.gates = ["มโนรมย์", "ช่องแค", "โคกกระเทียม", "เริงราง"]
...
predict_interval_label = st.sidebar.selectbox("เลือกเวลาทำนาย", interval_labels, index=0)
...
if page == "โหมดทดลอง (What-If)":
    ...
    with col2:
        ...
        cols = st.columns(5)
        st.session_state.water_levels = {st.session_state.gates[i]:10-i for i in range(4)}
        for i in range(4):
            with cols[i]:
    ...
    # print(water_levels)
    gate_names = ["C2", "Manorom", "Chong Khae", "Khok Krathiam", "Roeng Rang"]
    gate_heights = [0] + gate_levels
    gate_positions = [0, 1, 2, 3, 4]
